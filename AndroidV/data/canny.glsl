...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec2 sobeA = sobel(uv);
   	float sobelA_Strength = GetStrength(sobeA);
    float angle = GetAngle(sobeA);
    #if CANNY
    vec2 dir;
    ...
    float sobelB_Strength = GetStrength(sobelB);    
    float sobelC_Strength = GetStrength(sobelC);
...
#endif
    col += vec4(outlineCol*sobelA_Strength * INTENSITY, 1.0);
    // Output to screen
